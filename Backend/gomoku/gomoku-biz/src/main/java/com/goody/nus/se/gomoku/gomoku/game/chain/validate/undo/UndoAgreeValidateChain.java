...
/**
 * Validates that a player can agree to an undo proposal
 * Checks that there's a pending undo proposal
 * Checks that the player agreeing is NOT the one who proposed
 *
 * @author Haotian
...
@Slf4j
@Component
public class UndoAgreeValidateChain implements ValidateChain {
    @Override
    ...
    public boolean validate(GameDocument game, GameAction action) {
        ...
        if (!game.hasPendingUndoProposal()) {
            log.warn("No pending undo proposal to agree to");
            return false;
        }

        // Player agreeing must NOT be the one who proposed
        if (game.getUndoProposerColor() == action.getColor()) {
            log.warn("Player {} cannot agree to their own undo proposal", action.getColor());
            return false;
        }
