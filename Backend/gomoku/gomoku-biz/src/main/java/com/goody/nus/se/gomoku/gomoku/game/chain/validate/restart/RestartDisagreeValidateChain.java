...
/**
 * Validates that a player can disagree with a restart proposal
 *
 * <p>Validation rules:
 * <ul>
...
@Slf4j
@Component
public class RestartDisagreeValidateChain implements ValidateChain {
    ...
    /**
     * Validate RESTART_DISAGREE response
     *
    ...
    public boolean validate(GameDocument game, GameAction action) {
        ...
        if (!game.hasPendingRestartProposal()) {
            log.warn("[Restart] Validation failed: No pending restart proposal to disagree with");
            return false;
        }

        // Check that the responder is the opponent (not the proposer)
        if (game.getRestartProposerColor() == action.getColor()) {
            log.warn("[Restart] Validation failed: Proposer {} cannot disagree with their own proposal",
                    action.getPlayerId());
            return false;
        }

        log.debug("[Restart] Validation passed: Player {} can disagree with restart", action.getPlayerId());
        return true;
    }
