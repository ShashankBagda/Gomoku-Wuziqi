...
/**
 * Test for UndoDisagreeValidateChain
 *
 * @author Claude
 * @version 1.0
 */
class UndoDisagreeValidateChainTest {

    private UndoDisagreeValidateChain chain;
    private GameDocument game;

    @BeforeEach
    void setUp() {
        chain = new UndoDisagreeValidateChain();
    ...
    void testValidate_Success() {
        // BLACK proposed undo, WHITE disagrees
    ...
    @Test
    void testValidate_Success_WhiteProposedBlackDisagrees() {
        game.setUndoProposerColor(PlayerColor.WHITE);

        // WHITE proposed undo, BLACK disagrees
        GameAction action = GameAction.builder()
                .type(ActionType.UNDO_DISAGREE)
                .playerId(100L)
    ...
    @Test
    void testValidate_FailsWhenProposerTriesToDisagree() {
        // BLACK proposed undo, BLACK tries to disagree (invalid)
        GameAction action = GameAction.builder()
                .type(ActionType.UNDO_DISAGREE)
                .playerId(100L)
                .color(PlayerColor.BLACK)
    ...
    @Test
    void testValidate_FailsWhenWhiteProposerTriesToDisagree() {
        game.setUndoProposerColor(PlayerColor.WHITE);

        // WHITE proposed undo, WHITE tries to disagree (invalid)
        GameAction action = GameAction.builder()
                .type(ActionType.UNDO_DISAGREE)
                .playerId(200L)
                .color(PlayerColor.WHITE)
